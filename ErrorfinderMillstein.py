...
b1=[0.6,1.2]
c1=[0.3,0.6]
save_results_to_ito = Path("/home/carlv/Numerical Stochastic differential equations/Code_for_project_satellite/Results_Ito_Millstein")
for l in c1:
    for h in b1:
        for k in a1:
            a=k
            ...
            for j in range(mc):    
                ...
                for i in range(steps1):
                    Xnt=Xn+dt1*Yn
                    Yn=Yn-dt1*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW1[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW1[i]*dW1[i]-dt1)
                    Xn=Xnt
                ...
                for i in range(steps):
                    ...
                    Xnt=Xn+dt*Yn
                    Yn=Yn-dt*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW[i]*dW[i]-dt)
                    Xn=Xnt
                ...
                for i in range(steps):
                    ...
                    Xnt=Xn+dt2*Yn
                    Yn=Yn-dt2*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW2[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW2[i]*dW2[i]-dt2)
                    Xn=Xnt
                ...
                for i in range(steps):
                    ...
                    Xnt=Xn+dt3*Yn
                    Yn=Yn-dt3*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW3[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW3[i]*dW3[i]-dt3)
                    Xn=Xnt
                ...
                for i in range(steps):
                    ...
                    Xnt=Xn+dt4*Yn
                    Yn=Yn-dt4*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW4[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW4[i]*dW4[i]-dt4)
                    Xn=Xnt
                ...
                for i in range(steps):
                    ...
                    Xnt=Xn+dt5*Yn
                    Yn=Yn-dt5*(b*Yn+np.sin(Xn)-c*np.sin(2*Xn))-dW5[i]*(np.sin(Xn)+Yn*b)*a+0.5*(a**2)*(np.sin(Xn)+Yn*b)*b*(dW5[i]*dW5[i]-dt5)
                    Xn=Xnt
            ...
            plt.savefig(save_results_to_ito /f'Weakerror_a{a}_b{b}_c{c}_ItoMill.png', dpi = 300)
            #plt.show()
            plt.clf()
            ...
            plt.savefig(save_results_to_ito / f'L2error_a{a}_b{b}_c{c}_ItoMill.png', dpi = 300)
            #plt.show()
            plt.clf()
